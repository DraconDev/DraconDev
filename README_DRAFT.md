<!-- Proposed README draft. Intentionally differs from README.md while testing a cleaner product/pin/foundation structure. -->
# Hey, I make usable things people can run or play.

I build Chrome extensions, games, Rust infrastructure, terminal UX, Git utilities, and video workflows. Finished products and install/play pages are on [dracon.uk](https://dracon.uk).

**239K+ Rust lines** · **5.6K tests** · [Dracon crates on lib.rs](https://lib.rs/search?q=dracon)

When in doubt, let the stuff do the talking: shipped tools, install pages, and public code.

---

## Featured first: usable Chrome extensions and games

DraconDev mainly features usable things people can install, open, or play. Build-with foundations are secondary and stay below the product-facing items. Only verified install/play pages are linked.

<details><summary>Chrome extensions</summary>

- [**Auto Fullscreen**](https://chromewebstore.google.com/detail/auto-fullscreen-fullscree/liddomimembhlpgehhkhefijckmhkkjn) — fullscreen any page on load, navigation, or click-hold.
- Private browser extensions: no public install pages verified yet.

</details>

<details><summary>Games</summary>

- **Junk Runner** — private survival/trading roguelike; no public play page verified yet.

</details>

## Usable tools

Tools that solve a job directly:

- [**dracon-utilities**](https://github.com/DraconDev/dracon-utilities) — three installable Rust CLI utilities: `dracon-sync` for invisible Git sync, `dracon-system` for disk/process protection, and `dracon-warden` for Git secret encryption and repo hardening.
- [**folder-auto-banner**](https://github.com/DraconDev/folder-auto-banner) — contextual directory dashboard; `ls` with git, TODO, ports, Docker, build, and cached size context.
- [**tiles-tui-file-manager**](https://github.com/DraconDev/tiles-tui-file-manager) — dual-pane TUI file manager with git, SSH, editor, and system monitor.
- [**obs-wayland-hotkey**](https://github.com/DraconDev/obs-wayland-hotkey) — OBS hotkey daemon for Wayland/X11; crates.io + CI.
- [**youtube-video-uploader**](https://github.com/DraconDev/youtube-video-uploader) — Rust YouTube API client + CLI for resumable uploads.

## GitHub pin focus

Pinned work should be concrete, not a vague parent bucket. Current pin direction:

- [`dracon-sync`](https://github.com/DraconDev/dracon-utilities/tree/main/dracon-sync) — invisible Git sync for AI-powered development.
- [`dracon-system`](https://github.com/DraconDev/dracon-utilities/tree/main/dracon-system) — proactive disk/process guard.
- [`dracon-warden`](https://github.com/DraconDev/dracon-utilities/tree/main/dracon-warden) — Git secret encryption and repo hardening.
- [`tiles-tui-file-manager`](https://github.com/DraconDev/tiles-tui-file-manager) — concrete usable TUI file manager.
- [`folder-auto-banner`](https://github.com/DraconDev/folder-auto-banner) — contextual directory dashboard.
- [`obs-wayland-hotkey`](https://github.com/DraconDev/obs-wayland-hotkey) — concrete OBS hotkey daemon with Wayland/X11 value.

## Build-with foundations

These are useful if you are building similar tools, but they are not the main feature.

- [**dracon-terminal-engine**](https://github.com/DraconDev/dracon-terminal-engine) — TUI engine; 43 widgets for building terminal UX.
- [**dracon-ai-lib**](https://github.com/DraconDev/dracon-ai-lib) — Rust AI consumer library for BYOK tools and agents.
- [**azumi-live-ssr-framework**](https://github.com/DraconDev/azumi-live-ssr-framework) — live SSR framework for Rust on Axum; ~10KB gzipped.
- [**ai-gui-auto-video-editor**](https://github.com/DraconDev/ai-gui-auto-video-editor) — AGAVE: AI-assisted GUI video editor; product demo, not first-screen library.

[🌐 dracon.uk](https://dracon.uk) · [🎥 YouTube](https://youtube.com/@DraconDev) · [💰 Sponsor](https://github.com/sponsors/DraconDev)
